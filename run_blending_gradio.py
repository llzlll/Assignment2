...
def create_mask_from_points(points, img_h, img_w):
    ...
    pil_image = Image.new('L', (img_w, img_h), 0)
...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    laplacian_filter = torch.tensor([[0, 1, 0],[1, -4, 1],[0, 1, 0]], dtype=torch.float32, device=device).unsqueeze(0).unsqueeze(0)  
    laplacian_filter = laplacian_filter.repeat(3, 1, 1, 1).to(foreground_img.device)  

    fg_laplacian = F.conv2d(foreground_img, laplacian_filter, padding=1, groups=3)
    blended_laplacian = F.conv2d(blended_img, laplacian_filter, padding=1, groups=3)

    total_mask = foreground_mask + background_mask
    fg_laplacian_masked = fg_laplacian * total_mask
    blended_laplacian_masked = blended_laplacian * total_mask

    loss = (fg_laplacian_masked - blended_laplacian_masked).pow(2).sum() / (total_mask.sum() + 1e-6)
